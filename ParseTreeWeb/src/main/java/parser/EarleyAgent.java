...
/**
 * A class that keeps track of each step in Earley's algorithm for parsing
 * CFG's. We name it "EarleyAgent" as opposed to "EarleyRecord" because each
 * instance is highly automated, being endowed with the ability to sense where
 * the Early Algorithm is at and produce the next instances.
 * 
 * The algorithm for parsing is also embedded here, as a static method.
 * 
...
public class EarleyAgent implements Comparable {
	...
	/**
	 * return true if parse has succeed to get the RHS completely to the left of
	 * the break point
	 */
	boolean isDone() {
	...
	/**
	 * Rule always starts looking like: A --> (i)(i).XYZ unless j is specified
	 */
	...
	public boolean canComplete(EarleyAgent prefix) {
		return isDone() && J > I && I == prefix.J && !prefix.isDone()
				&& prefix.RHS[1].charAt(0) == LHS;
	}
	...
	public static void INITIALIZE(CFG g, String x)
			throws ExceptionInInitializerError, MalformedGrammarException {
		INPUT = x;
		GRAMMAR = g;
		RECORD = new TreeSet[x.length() + 1];
		if (GRAMMAR.hasEpsilons())
			/*
			 * FOR NOW THE FOLLOWING COMMENTED OUT B/C EXPERIMENTING WITH
			 * BEHAVIOR WHEN EPSILON TRANSITIONS ARE ALLOWED throw new
			 * MalformedGrammarException("Must not have epsilons!");
			 */
	...
	/**
	 * 0. "INITIALIZE" Set up the first set of records before starting with the
	 * while-loop
	 */
	...
	public static void completeEarley(int j) {
		...
		while (q.size() != 0) {
			...
			while (recItr.hasNext()) {
				...
				if (combination == null)
					continue;
				if (RECORD[j].add(combination)) { // true if rule was newly
													// created
	...
	public static EarleyAgent getEarleyAgent(String expression)
			throws MalformedGrammarException,
			ExceptionInInitializerError {
		proofOfSuccess=null;

		String input = expression.replaceAll("\\s", "");
